...
- Límite de subida de **4 MB** por archivo, centralizado en `src/lib/knowledge-constants.ts` y coherente entre frontend, validaciones y backend. Es un máximo seguro mientras la subida pasa por Route Handlers de Vercel (body de funciones serverless ~4.5 MB); **archivos mayores requerirán una futura subida directa a Vercel Blob** (client-upload). Al superar el límite se muestra un mensaje claro tanto en el navegador como en el servidor.
- Permisos del Centro de conocimiento: **OWNER** y **ADMIN** administran (subir, procesar, reintentar, reemplazar, renombrar, recategorizar, activar/desactivar y eliminar); **AGENT** solo consulta los documentos a través del agente (no administra archivos); **VIEWER** queda en solo lectura. Todo validado en el servidor, no solo ocultando botones.
